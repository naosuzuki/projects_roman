#!/usr/bin/env python3
"""
09_snia_zhist.py -- redshift histogram of ELAIS-N1 Type Ia SNe, each bar split
by peak Roman Z-band magnitude: blue = Z < 24 (PFS-observable live SNe),
red = Z >= 24 (too faint at peak; undefined/high-z sentinels included here).

    python 09_snia_zhist.py
"""
import os
import numpy as np
...
def main():
    ...
    print(f"ELAIS-N1 SNe Ia: {len(z)}  (Z<24: {blue.sum()}, Z>=24/undef: {red.sum()})")
    ...
    ax.set_ylabel("Number of SNe Ia", fontsize=18)
    ax.set_title(f"ELAIS-N1 Type Ia SNe vs Redshift  ($N={len(z):,}$)", fontsize=16)
    ax.tick_params(labelsize=13)
    ax.set_xlim(0, 3.0)
    ax.legend(fontsize=13, loc="upper right")
    ax.grid(True, axis="y", alpha=0.3)

    png = os.path.join(PNG_DIR, "09_snia_zhist_ELAIS-N1.png")
    fig.tight_layout()
