#!/usr/bin/env python3
"""
16_cc_specz_zhist.py -- redshift histogram of core-collapse (non-Ia) SNe that
obtain a successful live PFS spectroscopic redshift in the ELAIS-N1 survey, for
three weather-factor cases: 100% (no weather loss), 80%, and 70% of usable
nights. (CC analog of 13_snia_specz_zhist.py.)
...
def main():
    ...
    m = (typ == "CC") & (obs == 1)
    zo = z[m]
    print(f"successful CC SN spec-z: {len(zo)}  (z {zo.min():.3f}-{zo.max():.3f})")
    ...
    ax.set_ylabel("Number of CC SNe (Successful Spec-$z$)", fontsize=18)
    ax.set_title("ELAIS-N1 Core-Collapse SNe with Successful PFS Spec-$z$ vs Redshift",
                 fontsize=15)
    ...
    png = os.path.join(PNG_DIR, "16_cc_specz_zhist_ELAIS-N1.png")
    fig.tight_layout()
