#!/usr/bin/env python3
"""
12_cc_host_zhist.py -- redshift histogram of ELAIS-N1 core-collapse SN host
galaxies, each bar split three ways by host Roman Z-band magnitude:
  blue = Z < 24,  green = 24 <= Z < 25.5,  red = Z >= 25.5.

    python 12_cc_host_zhist.py
"""
import os
import numpy as np
...
CSV = os.path.join(HERE, "outputs", "csv", "03_cc_host_radec_ELAIS-N1.csv")
...
def main():
    ...
    print(f"ELAIS-N1 CC SN hosts: {len(z)}  "
    ...
    ax.set_title(f"ELAIS-N1 Core-Collapse SN Host Galaxies vs Redshift  "
                 f"($N={len(z):,}$)", fontsize=15)
    ax.tick_params(labelsize=13)
    ax.set_xlim(0, 3.0)
    ax.legend(fontsize=13, loc="upper right")
    ax.grid(True, axis="y", alpha=0.3)

    png = os.path.join(PNG_DIR, "12_cc_host_zhist_ELAIS-N1.png")
    fig.tight_layout()
