#!/usr/bin/env python3
"""
05_visibility.py -- visibility of a field (default ELAIS-N1) from the Subaru
site (Maunakea) over a year.
...
grid and shows:
  ...
  * bottom: airmass map over the year vs HST clock time, with twilight edges

    python 05_visibility.py                       # ELAIS-N1, 2029
    python 05_visibility.py --ra 242.5 --dec 54.5 --year 2030
"""
import os
import argparse
import warnings
...
ELAIS_N1 = (242.5, 54.5)
HST_OFFSET = 10.0           # HST = UTC - 10 (no DST)
...
def main():
    ...
    try:
        ...
        site = EarthLocation(lat=19.8255 * u.deg, lon=-155.4761 * u.deg, height=4139 * u.m)
    ...
    utc_h = np.arange(4.0, 16.001, 0.25)              # UTC 04-16 = HST 18:00-06:00
    dates = base + day_off * u.day                    # evening date (UTC midnight)
    # night belongs to UTC (date+1) 04:00..16:00
    ...
    print(f"{args.name}  (RA {args.ra}, Dec {args.dec})  from Subaru, {args.year}")
    print(f"  transit altitude ~ {90-abs(args.dec-site.lat.deg):.1f} deg "
    ...
    # CSV
    csv = os.path.join(CSV_DIR, f"05_visibility_{args.name}_{args.year}.csv")
    ...
    axtop.fill_between(dnum, hrs_obs, color="#3b7dd8", alpha=0.85,
    ...
    axtop.set_title(f"Visibility of {args.name} from Subaru (Maunakea), {args.year}  "
    ...
    X, Y = np.meshgrid(dnum, hst, indexing="ij")
    pm = axmap.pcolormesh(X, Y, np.clip(am_plot, 1, 3), cmap="viridis_r",
                          vmin=1, vmax=3, shading="nearest")
    # contours: airmass 1.5 and 2 (target alt), and twilight (sun alt -18, 0)
    ...
    axmap.set_ylabel("HST clock time", fontsize=LABEL_FS)
    ...
    png = os.path.join(PNG_DIR, f"05_visibility_{args.name}_{args.year}.png")
