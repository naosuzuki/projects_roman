#!/usr/bin/env python3
"""
14_snia_host_specz_zhist.py -- redshift histogram of SN Ia HOST galaxies that
obtain a successful PFS spectroscopic redshift (host integrated to S/N=5 at
10-pixel binning) in the ELAIS-N1 survey, for three weather-factor cases:
100% (no weather loss), 80%, and 70% of usable nights.
...
07_fiber_budget.py (column `host_completed`, `z` = host/SN redshift).
    ...
    python 14_snia_host_specz_zhist.py
"""
import os
import numpy as np
...
def main():
    ...
    m = (typ == "Ia") & (comp == 1)
    zo = z[m]
    print(f"successful SN Ia host spec-z: {len(zo)}  (z {zo.min():.3f}-{zo.max():.3f})")
    ...
    png = os.path.join(PNG_DIR, "14_snia_host_specz_zhist_ELAIS-N1.png")
    fig.tight_layout()
